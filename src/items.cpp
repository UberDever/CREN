...
namespace items
{
    ...
    {
        FILE* in; in = fopen(attrsPath, "r"); if (!in) return ITM_PARSE_ERR;
        ...
        while (true)
        {
            ...
            else
            {
                ...
                switch (hashName)
                {
                    ...
                    //case
                    default: std::cout << name << " token name\n"; return ITM_ATTR_ERR;
                }
                while (sscanf(token, "%[^=]s", attr) != -1)
                {
                    ...
                    switch (hashAttr) { //only attr can have attrs
                        case 93303930: //id
                        {
    ...
    {
        FILE* in; in = fopen(DBPath, "r"); if (!in) return ITM_PARSE_ERR;
        ...
        while (true)
        {
            ...
            else
            {
                ...
                while (sscanf(token, "%[^=]s", attr) != -1)
                {
                    ...
                    switch (hashAttr) { //only item can have attributes
    ...
    {
        if (attr.val.x > attr.cap.x) attr.val.x = attr.cap.x;
        if (attr.val.y > attr.cap.y) attr.val.y = attr.cap.y;
        if (attr.single)
        {
    ...
    BASE_ITEM constructItem(int ilvl, ITEM_TYPES possibleType)
    {
        ...
        if (curChance < magicChance)
        {
            ...
            if (grade > (attrNums - 1)) grade = attrNums - 1;
    ...
    e_exitCodes items_init() {
        ...
        //for (int i = 0; i < 15; i++){
        //    BASE_ITEM item = constructItem(60, static_cast<ITEM_TYPES>(THING | OHWEAP | CHEST | HELM)); //THING | OHWEAP | THWEAP |
